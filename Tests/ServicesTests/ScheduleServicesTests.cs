...
namespace Tests.ServicesTests
{
    public class ScheduleServicesTests
    {
        ...
        public async Task FetchSchedulesFromBackendAsync_ShouldReturnScheduleDtos()
        {
            ...
            {
                Schedule.CreateSchedule(Guid.NewGuid(), "Schedule 1"),
                Schedule.CreateSchedule(Guid.NewGuid(), "Schedule 2")
            };
        ...
        //     _scheduleRepositoryMock.Setup(x => x.GetScheduleByIdAsync(scheduleId))
        ...
        {
            // Arrange
            var scheduleDto = new CreateScheduleDto { Name = "New Schedule" };
            ...
            _scheduleRepositoryMock.Verify(x => 
                x.AddAsync(It.Is<Schedule>(s => s.Name == "New Schedule"), 1), Times.Once);
        ...
        public async Task EditSchedule_ValidScheduleDto_ShouldUpdateSchedule()
        {
            // Arrange
            var scheduleId = Guid.NewGuid();
            var existingSchedule = Schedule.CreateSchedule(scheduleId, "Old Name");
            var scheduleDto = new ScheduleDto { Id = scheduleId, Name = "Updated Name" };
            ...
            // Assert
            _scheduleRepositoryMock.Verify(x => x.GetScheduleByIdAsync(scheduleId), Times.Once);
        ...
        public async Task EditSchedule_SameName_ShouldNotUpdate()
        {
            // Arrange
            var scheduleId = Guid.NewGuid();
            var existingSchedule = Schedule.CreateSchedule(scheduleId, "Same Name");
            var scheduleDto = new ScheduleDto { Id = scheduleId, Name = "Same Name" };
        ...
        public async Task EditSchedule_ScheduleNotFound_ShouldThrowArgumentException()
        {
            // Arrange
            var scheduleId = Guid.NewGuid();
            var scheduleDto = new ScheduleDto { Id = scheduleId, Name = "Not Found" };
            _scheduleRepositoryMock.Setup(x => x.GetScheduleByIdAsync(scheduleId))
            ...
            Assert.Contains($"Schedule with id {scheduleId} not found", exception.Message);
        }
        ...
        public async Task DeleteSchedule_ValidScheduleDto_ShouldDeleteSchedule()
        {
            // Arrange
            var scheduleId = Guid.NewGuid();
            var existingSchedule = Schedule.CreateSchedule(scheduleId, "To Delete");
            var scheduleDto = new ScheduleDto { Id = scheduleId, Name = "To Delete" };
        ...
        {
            // Arrange
            var scheduleDto = new CreateScheduleDto { Name = "Test" };
            _unitOfWorkMock.Setup(x => x.SaveChangesAsync(It.IsAny<CancellationToken>()))
