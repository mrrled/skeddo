...
public class LessonDraftServicesTests
{
    ...
    public LessonDraftServicesTests()
    {
        ...
        _lessonDraftServices = new LessonDraftServices(
            ...
            new Logger<LessonDraftServices>(new LoggerFactory())
        );
    }
    ...
    public async Task GetLessonDraftById_ReturnsDto()
    {
        ...
        var scheduleId = Guid.NewGuid();
        var mathSubject = new SchoolSubject(Guid.NewGuid(), "Math");
        var lessonNumber = LessonNumber.CreateLessonNumber(1, "8:00-8:45").Value;
        var teacher = Teacher.CreateTeacher(Guid.NewGuid(), "John", "Doe", "DJ", [], []).Value;
        var studyGroup = new StudyGroup(Guid.NewGuid(), scheduleId, "Group A", []);
        ...
        // Act
        var taskResult = await _lessonDraftServices.GetLessonDraftById(lessonId);
    ...
    public async Task EditDraftLesson_WithMissingClassroom_UpdatesDraftAndReturnsDowngraded()
    {
        ...
        var scheduleResult = Schedule.CreateSchedule(scheduleId, "Test Schedule");
        ...
        var teacherResult = Teacher.CreateTeacher(teacherId, "John", "Doe", "DJ", [], []);
        var teacher = teacherResult.Value;
        var studyGroup = new StudyGroup(studyGroupId, scheduleId,"Group A", []);
        ...
        // Act
        var taskResult = await _lessonDraftServices.EditDraftLesson(lessonDraftDto, scheduleId);
        var result = taskResult.Value;
    ...
    public async Task EditDraftLesson_WithCompleteDataButFactoryFails_ThrowsException()
    {
        ...
        _teacherRepositoryMock
            .Setup(repo => repo.GetTeacherByIdAsync(teacherId))
            .ReturnsAsync(Teacher.CreateTeacher(teacherId, "John", "Doe", "DJ", [], []).Value);

        _studyGroupRepositoryMock
            .Setup(repo => repo.GetStudyGroupByIdAsync(studyGroupId))
            .ReturnsAsync(new StudyGroup(studyGroupId, scheduleId, "Group A", []));
